...
export function EventListItem({ event, onClick }: EventListItemProps) {
  ...
  return (
    <button
      ...
      className="group flex w-full items-start gap-3 rounded-[14px] bg-white/[0.03] px-3 py-3 text-left transition hover:bg-white/[0.05]"
    >
      <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-[12px] bg-white/[0.04]">
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-center gap-2">
          ...
          {upcoming ? (
            <span className="shrink-0 rounded-full bg-white/[0.05] px-1.5 py-0.5 text-[10px] text-[var(--text-muted)]">Upcoming</span>
          ) : null}
        </span>
